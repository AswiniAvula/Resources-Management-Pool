...
public class EmployeeController {
    ...
    public String updateDetailsForm(@PathVariable("id") long id, Model model, @ModelAttribute("employee") Employee employee) {
        try {
            Employee oldEmployeeData = employeeService.getEmployeeById(id);
            employee.setBlock("true");
            employee.setBlockCount(oldEmployeeData.getBlockCount());
...
//        System.out.println(managerList.get(0).getSelected());  
   ...
   try {
    ...
    public String saveEmployee(@ModelAttribute("employee") Employee employee, Model model) {
        try {
            //checking whether the employee details present or not
            if (employeeService.getEmployeeById(employee.getId()) == null) {
                //If employee not present. Save it.
                employee.setBlock("true");
                ...
                //If employee details present
                model.addAttribute("msg", "Employee already present");
                model.addAttribute("employee", employee);
            }
            return "New_employee";
        } catch (Exception e) {
   ...
   //update details
    ...
    try {
        ...
        for (int i = 1; i <worksheet.getLastRowNum(); i++) {
            ...
            Float employeeId = Float.valueOf(row.getCell(0).toString());
            Employee employee = new Employee();
            employee.setId(employeeId.longValue());
